...
int main()
{
    ...
    Grid.computeAABB(triag,triangleCount ,spheres ,sphereCount );
    /* Compute the Grids Dimensions */
    float ratio=10.0; // the higher the denser the grid
    Grid.computeDimensions(ratio);
    ...
    /* OpenCL Buffers */
    cl_mem colorBuffer, randomBuffer, sphereBuffer, triangleBuffer , GridBuffer;
    ...
    /* Read the Kernel */
    program_handle = fopen("radianceGPU.cl", "r");       // YOUR PATH TO KERNEL
    if (!program_handle)
    ...
    err = clSetKernelArg(kernel, 0, sizeof(cl_mem),(void *) &GridBuffer);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Failed to set OpenCL kernel arg. GridBuffer: %d\n", err);
        exit(-1);
    }

    err = clSetKernelArg(kernel, 1, sizeof(cl_mem),(void *) &colorBuffer);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Failed to set OpenCL kernel arg. colorBuffer: %d\n", err);
        exit(-1);
    }
    err = clSetKernelArg(kernel, 2, sizeof(cl_mem),(void *) &sphereBuffer);
    if (err != CL_SUCCESS) {
    ...
    err = clSetKernelArg(kernel, 3, sizeof(cl_mem),(void *) &triangleBuffer);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Failed to set OpenCL kernel arg. triangleBuffer: %d\n", err);
        exit(-1);
    }
    
    err = clSetKernelArg(kernel, 4, sizeof(cl_mem),(void *) &randomBuffer);
    if (err != CL_SUCCESS) {
    ...
    err = clSetKernelArg(kernel, 6, sizeof(int),(void *) &height);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Failed to set OpenCL kernel arg. height: %d\n", err);
        exit(-1);
    }
    
    err = clSetKernelArg(kernel, 7, sizeof(unsigned int),(void *) &sphereCount);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "Failed to set OpenCL kernel arg. sphereCount: %d\n", err);
        exit(-1);
    }
    
    err = clSetKernelArg(kernel, 8, sizeof(unsigned int),(void *) &triangleCount);
    if (err != CL_SUCCESS) {
    ...
    std::cout<<"Kernel Computation Time: "<<std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count() <<" ms"<<std::endl;
    std::cout<< "Writing to file " << std::endl;

    FILE *f = fopen("result_image_grid.ppm", "w");         // Write image to PPM file.   // YOUR PATH TO IMAGE
    fprintf(f, "P3\n%d %d\n%d\n", width, height, 255);

...
int main(int argc, char *argv[]) {
    ...
    // Получаем список устройств нужного типа
    ...
    context = clCreateContextFromType(cps, DEVICE_TYPE, NULL, NULL, &status);
    LOG_OCL_ERROR(status, "clCreateContextFromType Failed..." );

    // Создаем командную очередь
    commandQueue = clCreateCommandQueue(context, device_list[0], 0, &status);
    LOG_OCL_ERROR(status, "clCreateCommandQueue Failed..." );

    // Создаем входной буффер сразу с копированием данных
    ...
    clBufferSrc = clCreateBuffer( context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_uint) * NUM_OF_ELEMENTS, hostBuffer, &status);
    ...
    clBufferDst = clCreateBuffer( context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_uint) * NUM_OF_ELEMENTS, copyBuffer, &status);
    LOG_OCL_ERROR(status, "clCreateBuffer Failed..." );

    //Copy the contents of the rectangular buffer pointed by clBufferSrc to  clBufferSrc
    // Копируем содержимое квадратного буффера, указываем регионы копирования
    size_t   src_origin[3] = {0, 4, 0};
    size_t   dst_origin[3] = {0, 1, 0};
    size_t   region[3]     = {3*sizeof(int), 2, 1}; // Ширина в байтах, высота в строках, глубина в слайсах массивов
